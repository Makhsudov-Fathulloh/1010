...
class UserController extends Controller
{
    ...
    public function show(User $user, Request $request, OrderSearch $orderSearch)
    {
        $user->load('userDebt');
        ...
        if (Auth::user()->role->title == 'Moderator') {
            if ($user->role->title == 'worker' || $user->id == Auth::id()) {
                ...
                abort(403, 'Siz faqat worker foydalanuvchilarni ko‘ra olasiz.');
            }
        }
    ...
    public function export(Request $request)
    {
        ...
        return ExportService::export($query, 'users');
    }
