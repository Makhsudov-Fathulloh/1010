                                                ...
                                                <select name="items[{{ $i }}][product_variation_id]"
                                                    ...
                                                    @foreach($variations as $v)
                                                        <option value="{{ $v->id }}"
                                                            data-price="{{ $v->price }}"
                                                            data-rate="{{ $v->rate }}"
                                                            {{ ($item['product_variation_id'] ?? '') == $v->id ? 'selected' : '' }}>
                                                            {{ $v->title }} — {{ $v->product->title }} ({{ number_format($v->price_uzs, 0, '', ' ') }} сўм) [{{ \App\Helpers\CountHelper::format($v->count, $v->unit) }}]
                                                         </option>
                                                    @endforeach
                                                </select>
                                            </td>
                                                ...
                                                <select name="items[{{ $i }}][product_variation_id]"
                                                    ...
                                                    @foreach($variations as $v)
                                                        <option value="{{ $v->id }}"
                                                            ...
                                                            ({{ number_format($v->price_uzs, 0, '', ' ') }} сўм)
