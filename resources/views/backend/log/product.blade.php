...
<x-backend.layouts.main title="{{ 'Омборга кирим' }}">
    ...
    <script>
        document.getElementById('productLogFilterForm').addEventListener('submit', function (e) {
            const desktopSelect = this.querySelector('div.d-none.d-md-block select[name="filters[product_variation_id]"]');
            const mobileSelect = this.querySelector('div.d-md-none select[name="filters[product_variation_id]"]');

            if (desktopSelect && desktopSelect.offsetParent !== null) {
                // Desktop ko'rinib turgan bo'lsa, mobil selectni disable qil
                if (mobileSelect) mobileSelect.disabled = true;
            } else if (mobileSelect && mobileSelect.offsetParent !== null) {
                // Mobil ko'rinib turgan bo'lsa, desktop selectni disable qil
                if (desktopSelect) desktopSelect.disabled = true;
            }

            // Bo'sh input/selectlarni olib tashlash
